...
void SplatmapMix(Input IN, out half4 splat_control, out half weight, out fixed4 mixedDiffuse, inout fixed3 mixedNormal, out float emissive)
{
    ...
    mixedDiffuse = 0.0;
    mixedDiffuse += lerp(splat_control.r * tex2D(_Splat0, IN.uv_Splat0), splat_control.r * tex2D(_Splat1, IN.uv_Splat1), lrp);
    mixedDiffuse += lerp(splat_control.b * tex2D(_Splat2, IN.uv_Splat2), splat_control.b * tex2D(_Splat3, IN.uv_Splat3), lrp);

    emissive = (1.0-saturate((len-_Offset) + ns * _Interpolation) - lrp) * _ColorDisso.rgb*(_ColorDisso.a*50.0);
