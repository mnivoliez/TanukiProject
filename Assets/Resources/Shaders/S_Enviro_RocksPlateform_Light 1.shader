...
Shader "Shader Forge/hatching" {
    Properties {
        ...
        _LeavesTextures ("Leaves Textures", 2D) = "white" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _LeavesTextures; uniform float4 _LeavesTextures_ST;
...
////// Lighting:
                ...
                float3 node_7196 = saturate(( (lerp(_Basetexture_var.rgb,_ShadowColor.rgb,(floor(node_4818 * _ShadowEffects) / (_ShadowEffects - 1)*node_4845))*_Basetexture_var.a) > 0.5 ? (1.0-(1.0-2.0*((lerp(_Basetexture_var.rgb,_ShadowColor.rgb,(floor(node_4818 * _ShadowEffects) / (_ShadowEffects - 1)*node_4845))*_Basetexture_var.a)-0.5))*(1.0-lerp(_Globalcolor.rgb,_ShadowColor.rgb,(node_4845*(0.0*node_4818*_Thickness))))) : (2.0*(lerp(_Basetexture_var.rgb,_ShadowColor.rgb,(floor(node_4818 * _ShadowEffects) / (_ShadowEffects - 1)*node_4845))*_Basetexture_var.a)*lerp(_Globalcolor.rgb,_ShadowColor.rgb,(node_4845*(0.0*node_4818*_Thickness)))) ));
                float4 _LeavesTextures_var = tex2D(_LeavesTextures,TRANSFORM_TEX(i.uv0, _LeavesTextures));
                float3 finalColor = (lerp(node_7196,(_LeavesTextures_var.rgb*(_LeavesTextures_var.a*_Alphaintensity)),max(0,dot((normalDirection*_Emissiveheight).g,_Textureintensity)))*node_7196);
